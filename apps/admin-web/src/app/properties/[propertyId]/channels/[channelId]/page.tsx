  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <h1 className="mt-2 text-xl font-semibold tracking-tight text-slate-900">{channel.name}</h1>
        <p className="text-xs uppercase tracking-wide text-slate-400">{channel.type}</p>
      </div>
        ...
        <div className="space-y-5">
          <Card title={t('recentSyncs')}>
            {channel.recentJobs.length === 0 ? (
              <p className="text-sm text-slate-500">{t('noSyncs')}</p>
            ) : (
              <ul className="space-y-2 text-sm">
                {channel.recentJobs.map((job) => (
                  <li key={job.id} className="border-b border-slate-100 pb-2 last:border-0">
                    <div className="flex justify-between gap-2">
                      <span className="text-slate-700">{job.kind}</span>
                      <span
                        className={`text-xs font-medium ${JOB_TONE[job.status] ?? 'text-slate-500'}`}
                      >
            ...
            {channel.recentInbound.length === 0 ? (
              ...
              <ul className="space-y-2 text-sm">
                {channel.recentInbound.map((inbound) => (
                  <li key={inbound.id} className="border-b border-slate-100 pb-2 last:border-0">
                    <div className="flex justify-between gap-2">
                      {inbound.reservationId ? (
                        <Link
                          href={`/properties/${propertyId}/reservations/${inbound.reservationId}`}
                          className="tabular text-slate-800 underline-offset-2 hover:underline"
                        >
                      ...
                      <span
                        className={`text-xs font-medium ${
                          INBOUND_TONE[inbound.status] ?? 'text-slate-500'
                        }`}
                      >
